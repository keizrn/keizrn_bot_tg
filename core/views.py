...
bot_tg_wrong = {0: '😕 Я еще только учусь понимать людей, поэтому попробуй написать /help',
                ...
                5: 'Неправильный город, попробуйте снова.',
                6: 'Неправильный полином, попробуйте снова\nНажмите /start, чтобы выйти.',
                7: 'Ошибка! Делить на 0 нельзя.'
                }
...
@NataliaRU29 - Наталья Кей
@MariaZAGO - Мария З.'''


def continue_weather():
    return f'Попробуем еще город?\nНажмите /start, чтобы выйти.'
...
def choose_calc_type():
    return 'Вы можете работать с такими выражениями как:' \
           '* Рациональные (например: 2 + 2)' \
           '* Комплексные (например: 1+2j + 3+4j)' \
           '* Свободные (например: (1+2)-3*4)Выберите какие числа будем считать' \
           'Нажмите /start, чтобы выйти.'
...
def end_weather():
    return 'Рад был помочь. До встречи 🫶🏻'  # Сообщение при окончании вывода


def choose_weather_type():
    return 'Выберите тип поиска: по городу или по геолокации..\n' \
           '❗️ ВНИМАНИЕ функция "По геолокации" работает только на смартфоне 📱📲 при включенной геолокации 📡'  # Сообщение при выборе меню погода


def chosen_city():
    return 'Введите название города на русском языке..\nНажмите /start, чтобы выйти.'  # Сообщение при выборе поиска по городу
